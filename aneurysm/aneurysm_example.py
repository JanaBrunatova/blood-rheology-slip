...
if restart:
    if rank == 0:
        before_restart = os.path.join(folder, "before_restart")
        archive_root = os.path.join(folder, "before_restart_archive")

        # Set this to False if you prefer deleting the old before_restart
        ARCHIVE_OLD_BEFORE_RESTART = True

        # Handle an already existing before_restart
        if os.path.exists(before_restart):
            if ARCHIVE_OLD_BEFORE_RESTART:
                os.makedirs(archive_root, exist_ok=True)
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                archived_before_restart = os.path.join(
                    archive_root, f"before_restart_{timestamp}"
                )
                shutil.move(before_restart, archived_before_restart)
                print(f"Archived old before_restart -> {archived_before_restart}")
            else:
                shutil.rmtree(before_restart)
                print("Removed old before_restart")

        os.makedirs(before_restart, exist_ok=True)

        # Move restart files
        for fname in ("w.h5", "v_h5.h5"):
            source = os.path.join(folder, fname)
            if os.path.exists(source):
                shutil.move(source, os.path.join(before_restart, fname))
                print(f"Moved file {fname}")

        # Move subdirectories
        for entry in os.listdir(folder):
            if entry in ("before_restart", "before_restart_archive"):
                continue

            source = os.path.join(folder, entry)

            # Only move directories here, same idea as your original code
            if os.path.isdir(source):
                destination = os.path.join(before_restart, entry)
                shutil.move(source, destination)
                print(f"Moved directory {entry}")
