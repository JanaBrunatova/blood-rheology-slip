...
class CarreauPoiseuilleInflow(UserExpression):
    def eval(self, values, x):
        values[0] = 0.0
        values[1] = float(_u_interp(abs(x[0])))
...
def extract_quantites(domain_parts, bndry, Len, mesh) :
    ...
    #dx = Measure("dx", subdomain_data = domain_parts, domain=mesh)
...
#domain_parts, bndry, Len = mark_domain(mesh, boundary_parts, domain='full')
#print(f"{Len=}")
#rfull = extract_quantites(domain_parts, bndry, Len, mesh)
